...
public class Ejercicio2_CajaHorrible {
	...
	public static void main(String[] args) {
		...
		while (continuar.equalsIgnoreCase("S")) {
			...
			imprimirTicket(nombres, precios, unidades, subtotal, totales[0], totales[1], totales[2], totales[3],
					totales[4]);
	...
	static double[] calcularTotales(double subtotal, boolean esSocio) {
		...
		return new double[] { descuentoImporte, descuentoSocio, baseImponible, iva, total };
	}

	// método solo para imprimir el ticket
	static void imprimirTicket(String[] nombres, double[] precios, int[] unidades, double subtotal, double d1,
			double d2, double base, double iva, double total) {

		System.out.println("=========== TICKET ===========");

		for (int i = 0; i < nombres.length; i++) {
			System.out.println("LIN " + (i + 1) + " -> " + nombres[i] + " " + unidades[i] + " " + precios[i] + " "
